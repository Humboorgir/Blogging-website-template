...
export default function EditorForm({ categories, post }: EditorFormProps) {
  ...
  const [savedLabel, setSavedLabel] = useState(post ? "Saved just now" : "Unsaved changes");
            ...
            <section className="border border-border bg-background p-5 shadow-sm md:p-8">
              ...
              <div className="relative min-h-150 overflow-hidden">
                {preview ? (
                  <article
                    key="preview"
                    className={`min-h-150 max-w-2xl origin-top font-serif text-lg ${transitionDirection === "left" ? "animate-in fade-in slide-in-from-left-2" : "animate-in fade-in slide-in-from-right-2"}`}>
                    {previewContent}
                  </article>
                ) : (
                  <Textarea
                    ...
                    className={`min-h-150 w-full resize-y border-0 bg-transparent font-mono text-sm leading-7 outline-none ${errors.body ? "border-destructive" : ""}`}
